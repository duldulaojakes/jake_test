<%@ Page Title="" Language="C#" MasterPageFile="~/design/MasterPage.master" AutoEventWireup="true" CodeFile="ClassCategory.aspx.cs" Inherits="ClassCategory" %>
...
<artisteer:Block ID="MedicineName" Caption="[Drug Class Category]" runat="server"><ContentTemplate>
...
<artisteer:Article ID="Article1"  Caption="Welcome - Drug guide book(All Drugs Categorized by Classes)" runat="server"><ContentTemplate>
    ...
    <p>Drug abuse: , , Drug abuse is a serious public health problem that affects almost every community and family in some way. Each year drug abuse results in around 4 million serious illnesses or injuries among people in the Philippines.
Concern beliefs in medicines are patients’ anxieties about the harmful effects of a specific prescribed medication. Three papers examined the importance of concern beliefs in medicine, specifically its relationship to patient outcomes such as self-reported adverse drug events (ADEs) and symptom attribution, and the factors that might
 drive a change in concern beliefs over time. For the first and second paper, a cross-sectional internet survey of Medicare enrollees who were English speakers, 65 years and older and enrolled in the Medicare Part D program was done. In the third paper, a longitudinal internet survey of
the same sample was done before Medicare Part D in 2005 and after Medicare Part D in 2007, and adults 40 years and older with physical limitations were interviewed using telephone. Multiple logistic regressions showed that having stronger concern beliefs in medicine and more symptoms was related to self-reported ADE, rather than using an inappropriate medicine or the number of inappropriate medicines used.</p>
    ...
    <artisteer:Article ID="Article2" Caption="Drugs from A-Z: Categorized by Classes" runat="server"><ContentTemplate><p>Drug guide book is one example 
    for taking medicines guidelines. Web Applications has a big help for us in terms of transactions 
    and businesses. We can make a website for a specific field of study. 
    There are many existing Drug Guide Book today specially in other rich 
    countries which has a big help for people. The Electronic Drug Guide Book 
    in an application which we will be guided for medication upon browsing in the Internet.
     It is technologically designed and implemented to other countries and has a big purpose in guidelines for medication processes. Electronic Drug Guide Book is exciting medical information application for medical guidelines in the Internet to help improve your health care now and in the future, besides, it will keep you healthy and medically aware.</p>
    ...
    <asp:ScriptManager ID="ScriptManager1" runat="server">
        ...
        <asp:UpdatePanel ID="PleaseWaitPanel"  runat="server" RenderMode="Inline">
            ...
            WatermarkText="Drugname..">
                   ...
                   <asp:GridView ID="GridView5" runat="server" AutoGenerateColumns="False" Width="686px" 
                            DataKeyNames="DrugID" onrowcommand="GridView5_RowCommand" 
                            AllowPaging="True" onpageindexchanging="GridView5_PageIndexChanging" 
                            PageSize="50">
                    <Columns>
                        <asp:ButtonField CommandName="Select" DataTextField="DrugName" 
